...
def checker(csv_file='test.csv', json_model='model.json', h5_model='model.h5'):
    ...
    ##Num_words is equal to the vocab size on Test_confi.py so if the vocab size in confi.py is 10000, it will return error if you put 100000 here
    ...
    for i in range(0, len(post)):
        print(post.iloc[i][:50], "...")
        print("Document %s is %s sentiment; %f%% confidence" % (data['file_name'][i], text_labels[i], pred[i][np.argmax(pred[i])] * 100))
